...
var startCameraButton = document.querySelector('button#startCamera');
...
var constraintsSelect = document.querySelector('select#camera-constraints');
var zoomInput = document.querySelector('input#zoom');

startCameraButton.onclick = getStream;
...
function getStream() {
  ...
  const resolution_ = constraintsSelect.options[constraintsSelect.selectedIndex].text.split('x');
  const width_ = parseInt(resolution_[0]);
  const height_ = parseInt(resolution_[1]);

  var videoSource = videoSelect.value;
  constraints = {
    video: {deviceId: videoSource ? {exact: videoSource} : undefined,
            width: {ideal: width_},
            height: {ideal: height_}}
  };
